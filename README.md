...
- **Texture (image) brushes fall back to solid black**: brush type 2 (`EmfPlusTextureBrushData`, MS-EMFPLUS 2.2.2.45) is not parsed at all and falls through to the same default-black handling as any unrecognised brush type; the embedded bitmap and its `WrapMode` are not decoded into a Canvas pattern.
